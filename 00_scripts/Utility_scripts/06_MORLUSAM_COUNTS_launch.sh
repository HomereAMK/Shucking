...
# Clean up
rm 00_scripts/USAMMORLCOUNTS*sh
...
do
	base=$(basename "$file")
	toEval="cat 00_scripts/Utility_scripts/06_MORLUSAM_COUNTS.sh | sed 's/__BASE__/$base/g'"; eval $toEval > 00_scripts/DEPTHFASTQ_"$base".sh
done
...
#Submit jobs
for i in $(ls 00_scripts/USAMMORLCOUNTS*sh); do qsub $i; done
