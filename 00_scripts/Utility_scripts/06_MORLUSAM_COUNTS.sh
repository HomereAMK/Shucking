...
#PBS -o Shucking/98_log_files/Trim/__BASE__USAMMORLCOUNTS.out
#PBS -e Shucking/98_log_files/Trim/__BASE__USAMMORLCOUNTS.err
...
echo $(( $a + $b )) > Shucking/07_depth/USAM_MORL/"$base".USAM_MORL.count_fastq_1.tmp
#raw bases
c=`zcat Novaseq_MLX_USA/"$base"_R1.fastq.gz | awk 'NR%4==2' | tr -d "\n" | wc -m` 
d=`zcat Novaseq_MLX_USA/"$base"_R2.fastq.gz | awk 'NR%4==2' | tr -d "\n" | wc -m`
echo $(( $c + $d )) > Shucking/07_depth/USAM_MORL/"$base".USAM_MORL.count_fastq_2.tmp
#trim bases
...
RAWREADS=`cat Shucking/07_depth/USAM_MORL/"$base".USAM_MORL.count_fastq_1.tmp`
RAWBASES=`cat Shucking/07_depth/USAM_MORL/"$base".USAM_MORL.count_fastq_2.tmp`
...
printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" $base $RAWREADS $RAWBASES >> Shucking/07_depth/Summary_USAMMORLdepth_lcWGS_19oct22.txt
